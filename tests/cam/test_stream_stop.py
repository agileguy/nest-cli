"""Tests for ``cam stream-stop`` (FR-CAM-14).

Invalidate an active session via SDM ``StopRtspStream``. Missing
``--extension-token`` exits 64. SDM 4xx exits 1 (device error).
Successful stop exits 0.
"""
...
class TestStreamStopHappyPath:
    @responses.activate
    def test_stop_exits_0(self, fake_paths: dict[str, Path]) -> None:
        responses.add(
            responses.GET,
...
class TestStreamStopMissingToken:
    def test_missing_extension_token_exits_64(self, fake_paths: dict[str, Path]) -> None:
        runner = CliRunner()
        result = runner.invoke(cli_root, ["cam", "stream-stop", _TARGET, "--json"])
        assert result.exit_code == 64


class TestStreamStop4xx:
    @responses.activate
